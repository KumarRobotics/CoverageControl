...
namespace CoverageControl {
	...
	bool CheckCudaErrors(cudaError_t result) {
		if (result != cudaSuccess) {
			std::cerr << "CUDA error: [" << static_cast<unsigned int>(result) << "] " << cudaGetErrorString(result) << std::endl;
			return false;
		}
		return true;
	}
	...
	int CudaUtils::GPUDeviceInit(int dev_id) {
		...
		if (dev_id > device_count - 1) {
			...
			std::cerr << "Device ID: " << dev_id << " Device Count: " << device_count << std::endl;
			return -1;
		}

		int compute_mode = -1, major = 0, minor = 0;
		CheckCudaErrors(cudaDeviceGetAttribute(&compute_mode, cudaDevAttrComputeMode, dev_id));
		...
		if (major < 1) {
			std::cerr << "GPUDeviceInit(): GPU device does not support CUDA." << std::endl;
			return -1;
		}

		CheckCudaErrors(cudaSetDevice(dev_id));
		std::cout << "GPU Device " << dev_id << " has been set" << std::endl;
		std::cout << "CUDA Device [" << dev_id << "]: \"" << _ConvertSMVer2ArchName(major, minor) << "\"" << std::endl;
		is_cuda_initialized_ = true;
		device_id_ = dev_id;
		return dev_id;
	}

	int CudaUtils::FindDevice() {
	...
	int CudaUtils::FindIntegratedGPU() {
		...
		while (current_device < device_count) {
			...
			if (integrated && (compute_mode != cudaComputeModeProhibited)) {
				...
				std::cout << "GPU Device " << current_device << " has been set" << std::endl;
				std::cout << "CUDA Device [" << current_device << "]: \"" << _ConvertSMVer2ArchName(major, minor) << "\"" << std::endl;
		...
		if (devices_prohibited == device_count) {
			std::cerr << "CUDA error: No Integrated GPU found that supports CUDA." << std::endl;
			return -1;
		}
