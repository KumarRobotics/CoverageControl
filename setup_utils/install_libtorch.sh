...
if [[ ${NOCUDA} ]]
then
	wget https://download.pytorch.org/libtorch/cpu/libtorch-cxx11-abi-shared-with-deps-2.4.0%2Bcpu.zip -O ${TMP_DIR}/libtorch.zip

else
	wget https://download.pytorch.org/libtorch/cu121/libtorch-cxx11-abi-shared-with-deps-2.4.0%2Bcu124.zip -O ${TMP_DIR}/libtorch.zip
fi
