...
namespace CoverageControl {
	class BivariateNormalDistribution {
		...
		public:
			...
			BivariateNormalDistribution (Point2 const mean, double const sigma, double peak_val = 1) {
				is_circular_ = true;
			...
			BivariateNormalDistribution (Point2 const mean, Point2 const sigma, double rho, double peak_val = 1) {
				if(rho_ > 0) {
					is_circular_ = false;
				} else {
					is_circular_ = true;
