...
def write_npz(iRobot):
    np.savez_compressed('/home/saurav/CoverageControl_ws/data/cnn_data_3channel/data_' + f'{(count * num_robots + iRobot):07d}' + '.npz', local_map = env.GetRobotLocalMap(iRobot), communication_map = env.GetCommunicationMap(iRobot), exp_map = env.GetRobotExplorationMap(iRobot), label=np.concatenate((env.GetVoronoiCell(iRobot).centroid, [env.GetVoronoiCell(iRobot).mass])))
